...
const renderTasks = (taskList) => {
    ...
    taskList.forEach(task => {
        ...
        // Исправлено: правильное применение стиля завершенной задачи
        if (task.completed) {
        ...
        deleteButton.onclick = (e) => {
            e.stopPropagation();
            // БАГ: Удаляется всегда первая задача (оставлено для демонстрации)
            deleteTask(0);
        };
...
const toggleTask = (id) => {
    // Исправлено: работает переключение статуса
...
const deleteTask = (id) => {
    // БАГ: Фильтрация не работает правильно (оставлено для демонстрации)
...
const filterTasks = (status) => {
    ...
    let predicate;
    if (status === 'completed') {
        predicate = task => !task.completed; // БАГ: Инвертирована логика (оставлено для демонстрации)
